<!-- FIXTURE (must fail): the four time-budget rows sum to 14.5, not 15.0.
     Proves check-schema enforces the weekly hour total in the RENDERED file,
     which check-hours cannot see because it reads canon and not prose. -->
...
`T-w01-1` — 1.5h, Track A. Classify every stage of the platform pipeline as workflow-shaped or agent-shaped.

### Task 2

`T-w01-3` — 3.0h, Track A. Build S0: task file to git worktree to `claude -p` subprocess to captured diff.
