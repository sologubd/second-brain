<!-- FIXTURE (must fail): `### Task 2` (T-w01-3) is referenced by no acceptance
     criterion. Proves check-schema enforces the Tasks-to-Acceptance rule —
     the rule that stops a task from being work nobody ever has to show for. -->
...
- [ ] `make demo-s0 TASK=tasks/example.md` exits 0 and writes a diff file (T-w01-1)
- [ ] The classification note names 2 failure modes each choice avoids (T-w01-1)
